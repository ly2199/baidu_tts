...
**长文本播报不连续？**
1.0.1 版本已支持自动分段合成拼接，请确认已更新到最新版本。

## 版本历史

- **1.0.3**：新增集成品牌图标（`brand/` 目录，适配 HA 2026.3+ 品牌代理 API）
- **1.0.2**：修复"TTS engine name is not set"错误（TTS 引擎要求实体名称非空）
- **1.0.1**：适配 HA 2026.8（新版 TTS 实体 API、流式输出）；改用 aiohttp 连接复用；长文本自动分段拼接；令牌缓存跨重载保留；选项键 `speaker` 迁移为标准 `voice`
- **1.0.0**：首个版本
